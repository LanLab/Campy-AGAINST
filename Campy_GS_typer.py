...
#example run: python Campy_AGAINST.py --query TRIAL_QUERY --reference TRIAL_REFERENCE --thread 4 --output ./testing_output.txt
...
def ksi(fastANI_output, args):
    ...
    with open(args.output, "w") as output_file:
        output_file.write("Query Genome\tHighest ANI similarity\tReference Genome with the hignest ANI similarity\tReference Genome_ANI Genomic Species\tCampylobacter Genomic Species\n")
...
def parseargs():
    ...
    parser.add_argument("-r", "--reference", help="folder for the 60 medoid genomes",
                        required=True)
    parser.add_argument("-o", "--output", help="output_the_ani_genomeicpseices of each query ", required=True)
